...
export class GoogleDrive {
    ...
    async syncTrackingMap() {
        ...
        if (this.#debug) console.debug('[GoogleDrive.syncTrackingMap] tracking map synchronized');
    }
    #API_KEY = null;
    ...
    constructor(gitHubDataObject, options = {}) {
        console.debug('[GoogleDrive.constructor] ENTER: constructor', {gitHubDataObject, options});
        ...
        if (typeof options.retryCount === "number") this.#retryCount = options.retryCount;
        if (typeof options.backoffMs === "number") this.#backoffMs = options.backoffMs;
    ...
    static requireString(value, name) {
        if (typeof value !== "string" || value.trim() === "") {
            throw new Error(`GoogleDrive: Parameter '${name}' must be a non-empty string.`);
        }
    }
    /**
    ...
    async #fetchWithRetry(url, fetchOptions = {}, options = {}) {
        let attempt = 0;
        const retryCount = typeof options.retryCount === "number" ? options.retryCount : this.#retryCount;
        const backoffMs = typeof options.backoffMs === "number" ? options.backoffMs : this.#backoffMs;
        ...
        while (true) {
            try {
                ...
                if (!response.ok && response.status >= 500 && response.status < 600) {
                    ...
                    if (debug) console.warn(`[GoogleDrive.#fetchWithRetry] 5xx error, retrying...`, { url, status: response.status, attempt });
                ...
                if (attempt >= retryCount) throw new Error(`GoogleDrive: Network error after ${retryCount + 1} attempts: ${err.message}`);
                if (debug) console.warn(`[GoogleDrive.#fetchWithRetry] Network error, retrying...`, { url, attempt, err });
                await new Promise(res => setTimeout(res, backoffMs * Math.pow(2, attempt)));
                attempt++;
            }
        }
    ...
    get SecretValue() { return this.#secretValue; }
    // Property to get the Google Secret Manager resource name
    get SecretManagerName() { return this.#secretManagerName; }
    ...
    async fetchSecretManagerValue(options = {}) {
        ...
        const fetchOptions = {
            method: 'GET',
            headers: { 'Authorization': 'Bearer ' + this._gisToken, ...(options.headers || {}) },
    ...
    static async Factory(gitHubDataObject, config) {
        // ABSOLUTE FIRST LINE
        console.debug('[GoogleDrive.Factory] [ULTRA-EARLY] ABSOLUTE FIRST LINE of Factory');
        try {
            console.debug('[GoogleDrive.Factory] [ULTRA-EARLY] Inside try block, before any code');
            console.debug('[GoogleDrive.Factory] >>> ENTER Factory');
            console.debug('[GoogleDrive.Factory] DEBUG: Entered Factory, about to check config.');
            console.debug('[GoogleDrive.Factory] DEBUG: After config normalization. CLIENT_ID:', config && config.CLIENT_ID, 'DISCOVERY_DOCS:', config && config.DISCOVERY_DOCS, 'SCOPES:', config && config.SCOPES);
            console.log('[GoogleDrive.Factory] called', {gitHubDataObject, config});
            // Early error handling for null/invalid config
            console.debug('[GoogleDrive.Factory] [ULTRA-EARLY] Before config null/undefined check');
            if (config === null || config === undefined) {
                console.error('[GoogleDrive.Factory] ERROR: config is null or undefined. Aborting GoogleDrive.Factory early.');
                return null;
            }
            // Backward compatibility: if config has web property, use it for CLIENT_ID, etc.
            console.debug('[GoogleDrive.Factory] [ULTRA-EARLY] Before config.web check');
            if (config.web) {
                config.CLIENT_ID = config.CLIENT_ID || config.web.client_id;
                config.DISCOVERY_DOCS = config.DISCOVERY_DOCS || config.web.discovery_docs;
            ...
            console.debug('[GoogleDrive.Factory] [ULTRA-EARLY] Before hasValidGitHubData check');
            const hasValidGitHubData = gitHubDataObject && typeof gitHubDataObject.get === 'function';
            if (!hasValidGitHubData) {
                console.warn('[GoogleDrive.Factory] gitHubDataObject is missing or does not have a get() method. Falling back to local config.');
            }
            console.debug('[GoogleDrive.Factory] [ULTRA-EARLY] Before new GoogleDrive()');
            drive = new GoogleDrive(hasValidGitHubData ? gitHubDataObject : null);
            console.debug('[GoogleDrive.Factory] DEBUG: GoogleDrive instance created.');
            if (config) {
                ...
                console.debug('[GoogleDrive.Factory] DEBUG: Config applied to drive instance. CLIENT_ID:', drive.CLIENT_ID, 'SCOPES:', drive.SCOPES);
            } else {
                // Use robust GitHubData API for config and secrets if available, else fallback to local
                let googleConfig = null;
                let secrets = null;
                if (hasValidGitHubData && drive._gitHubDataObj && typeof drive._gitHubDataObj.get === 'function') {
                    try {
                        googleConfig = await drive._gitHubDataObj.get("googleDrive.json", "json", null, {});
                        if (!googleConfig) throw new Error('No googleConfig returned from GitHubData.get');
                        console.debug('[GoogleDrive.Factory] Loaded googleConfig:', googleConfig);
                    } catch (e) {
                        console.error('[GoogleDrive.Factory] Failed to load googleDrive.json from GitHubData:', e);
                    }
                    try {
                        secrets = await drive._gitHubDataObj.get("secrets.json", "json", null, {});
                        console.debug('[GoogleDrive.Factory] Loaded secrets:', secrets);
                    } catch(error) {
                        console.error('[GoogleDrive.Factory] Failed to load secrets.json from GitHubData:', error);
                    }
                }
                ...
                if (!googleConfig) {
                    try {
                        ...
                        console.debug('[GoogleDrive.Factory] Loaded googleConfig from local file:', googleConfig);
                    } catch(localError) {
                        googleConfig = undefined;
                        console.error('[GoogleDrive.Factory] Failed to load googleDrive.json from local file:', localError);
                    }
                }
                if (!secrets) {
                    try {
                        ...
                        console.debug('[GoogleDrive.Factory] Loaded secrets from local file:', secrets);
                    } catch(localError) {
                        secrets = undefined;
                        console.error('[GoogleDrive.Factory] Failed to load secrets.json:', localError);
                    }
                }
                if (!googleConfig || !googleConfig.web || !googleConfig.web.client_id) {
                    console.error('[GoogleDrive.Factory] googleConfig.web.client_id is missing or invalid:', googleConfig);
                }
                drive.CLIENT_ID = googleConfig && googleConfig.web ? googleConfig.web.client_id : undefined;
                drive.DISCOVERY_DOCS = googleConfig && googleConfig.web ? googleConfig.web.discovery_docs : undefined;
                drive.SCOPES = googleConfig && googleConfig.web ? googleConfig.web.scopes : undefined;
                if (googleConfig && googleConfig.web && googleConfig.web.name) drive._secretManagerName = googleConfig.web.name;
                if(secrets && secrets.googleDrive && secrets.googleDrive.client_secret) drive.API_KEY = secrets.googleDrive.client_secret;
            }
            ...
            console.debug('[GoogleDrive.Factory] Loading Google Identity Services script...');
            ...
            console.debug('[GoogleDrive.Factory] GIS script load elapsed ms:', Date.now() - debugStart);
            console.debug('[GoogleDrive.Factory] GIS script load result:', gisScriptResult);
            console.debug('[GoogleDrive.Factory] GIS script loaded. Checking sign-in status...');
            // Always ensure user is signed in before fetching secrets or returning instance
            if (!drive.isSignedIn || typeof drive.isSignedIn !== 'function' || !drive.isSignedIn()) {
                console.debug('[GoogleDrive.Factory] Not signed in. Calling signIn()...');
                const debugSignInStart = Date.now();
                try {
                    //await drive.signIn();
                    console.debug('[GoogleDrive.Factory] signIn() elapsed ms:', Date.now() - debugSignInStart);
            ...
            if (drive._secretManagerName && drive._gisToken) {
                ...
                if (drive.gisToken) {
                    ...
                    console.warn('[GoogleDrive.Factory] SecretManagerName is set but not signed in; skipping fetchSecretManagerValue.');
                }
            }
            console.log('[GoogleDrive.Factory] END OF Factory, about to return drive instance.');
            console.debug('[GoogleDrive.Factory] <<< EXIT Factory, returning drive instance');
            return drive;
        } catch (err) {
            console.error('[GoogleDrive.Factory] [ULTRA-EARLY] ERROR in Factory:', err);
            throw err;
        }
    }
    /*
...
accessSecret();
    ...
    async loadGisScript() {
        ...
        try {
            ...
            return await new Promise((resolve, reject) => {
                console.debug('[GoogleDrive.loadGisScript] Creating script tag for GIS...');
                ...
                script.onerror = (e) => {
                    console.error('[GoogleDrive.loadGisScript] GIS script failed to load:', e);
                    reject("Failed to load Google Identity Services script");
                };
                document.body.appendChild(script);
                console.debug('[GoogleDrive.loadGisScript] Script tag appended to body.');
            });
        } catch (err) {
    ...
    async signIn(silent = false) {
        console.debug('[GoogleDrive.signIn] >>> ENTER signIn. silent:', silent, 'CLIENT_ID:', this.CLIENT_ID, 'SCOPES:', this.SCOPES);
        try {
            return await new Promise((resolve, reject) => {
                if (!window.google || !window.google.accounts || !window.google.accounts.oauth2) {
                    console.error('[GoogleDrive.signIn] Google Identity Services OAuth2 not loaded');
                ...
                if (!this._tokenClient) {
                    ...
                    this._tokenClient = window.google.accounts.oauth2.initTokenClient({
                        ...
                        scope: this.SCOPES,
                        callback: (response) => {
                            console.debug('[GoogleDrive.signIn] Token client callback. Response:', response);
                ...
                if (silent) {
                    console.debug('[GoogleDrive.signIn] Requesting access token (silent)...');
                    this._tokenClient.requestAccessToken({ prompt: '' });
                } else {
                    console.debug('[GoogleDrive.signIn] Requesting access token (interactive)...');
    ...
    async uploadRawFile(name, content, mimeType = 'text/plain', options = {}) {
        ...
        if (typeof content !== "string" && !(content instanceof Blob)) throw new Error("Content must be a string or Blob");
        ...
        const res = await this.#fetchWithRetry('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id', fetchOptions, options);
        return res.json();
    }
    ...
    async downloadRawFile(fileId, options = {}) {
        ...
        const res = await this.#fetchWithRetry(`https://www.googleapis.com/drive/v3/files/${fileId}?alt=media`, fetchOptions, options);
        if (res.status === 404) {
            if (this.#debug) console.warn(`[GoogleDrive.downloadRawFile] File not found: ${fileId}`);
            return null;
        }
    ...
    async listDirectory(query = '', pageSize = 10, options = {}) {
        ...
        if (typeof pageSize !== 'number' || pageSize <= 0) throw new Error('pageSize must be a positive number');
        const accessToken = this._gisToken;
        if (!accessToken) throw new Error("Not signed in with Google");
        ...
        const res = await this.#fetchWithRetry(`https://www.googleapis.com/drive/v3/files?${params.toString()}`, fetchOptions, options);
    ...
    async get(fileId, type = "raw", options = {}) {
        GoogleDrive.requireString(fileId, "fileId");
        if (this.#debug) console.debug('[GoogleDrive.get] called', { fileId, type, options });
        ...
        if (entry && entry.expires) {
            ...
            if (!isNaN(exp) && now > exp) {
                if (this.#debug) console.warn('[GoogleDrive.get] file expired', { fileId, expires: entry.expires });
                throw new Error(`GoogleDrive.get: fileId ${fileId} is expired (expired at ${entry.expires})`);
            }
        }
        try {
            if (type === "raw") {
            ...
            if (err && err.message && err.message.includes('Failed to download file')) {
                if (this.#debug) console.warn(`[GoogleDrive.get] File not found or failed to download: ${fileId}`);
                return null;
            }
    ...
    async has(fileId, options = {}) {
        ...
        const res = await this.#fetchWithRetry(`https://www.googleapis.com/drive/v3/files/${fileId}?fields=id`, fetchOptions, options);
        return res.ok;
    }
    ...
    async batchExists(fileIds, options = {}) {
        if (!Array.isArray(fileIds)) throw new Error("GoogleDrive.batchExists: fileIds must be an array.");
        const results = {};
    ...
    async pruneExpiredFiles(options = {}) {
        if (options.enabled === false) {
            if (this.#debug) console.info('[GoogleDrive.pruneExpiredFiles] skipped (disabled by options)');
            return 0;
        }
        await this.syncTrackingMap();
        const now = Date.now();
        ...
        let pruned = 0;
        const results = await Promise.all(idsToDelete.map(async (fileId) => {
            try {
                await this.deleteFile(fileId);
                delete this.#trackingMap[fileId];
                if (this.#debug) console.info('[GoogleDrive.pruneExpiredFiles] deleted', fileId);
                pruned++;
                return true;
            } catch (err) {
                if (this.#debug) console.warn('[GoogleDrive.pruneExpiredFiles] failed to delete', fileId, err);
                return false;
            }
        }));
        ...
        if (this.#debug) console.info('[GoogleDrive.pruneExpiredFiles] total pruned', pruned, 'out of', idsToDelete.length);
        return pruned;
    }
    ...
    static getShardKey(fileId) {
        ...
        return fileId[0].toLowerCase().match(/[0-9a-f]/) ? fileId[0].toLowerCase() : '0';
    }
    ...
    async loadTrackingMap() {
            ...
            promises.push(
                (async () => {
                    try {
                        ...
                        if (this.#debug) console.warn('[GoogleDrive.loadTrackingMap] failed to load shard', name, err);
                    }
                })()
            );
        }
        ...
        if (this.#debug) console.debug('[GoogleDrive.loadTrackingMap] loaded all shards', all);
        ...
        for (let i = 0; i < this.#trackingMapShards; i++) {
            ...
            promises.push(
                this.set(name, payload, type).catch(err => {
                    if (this.#debug) console.error('[GoogleDrive.saveTrackingMap] failed to save shard', name, err);
                })
            );
        }
        await Promise.all(promises);
        if (this.#debug) console.debug('[GoogleDrive.saveTrackingMap] saved all shards' + (compress ? ' (compressed)' : ''));
    }
    /**
    ...
    async loadTrackingMap(options = {}) {
        ...
        await Promise.all(promises);
        this.#trackingMap = all;
        if (this.#debug) console.debug('[GoogleDrive.loadTrackingMap] loaded all shards' + (decompress ? ' (decompressed)' : ''), all);
        return this.#trackingMap;
    }
    ...
    async updateTrackingMap(fileId, meta = {}) {
        ...
        this.#trackingMap[fileId].versions = this.#trackingMap[fileId].versions || [];
        ...
        if (this.#debug) console.debug('[GoogleDrive.updateTrackingMap] versioned update (advanced metadata)', fileId, meta);
        await this.saveTrackingMap();
    }
    ...
    getFileVersions(fileId) {
        if (!fileId || !this.#trackingMap[fileId] || !Array.isArray(this.#trackingMap[fileId].versions)) return [];
        ...
        if (options.quotaWarn) {
            try {
                ...
                if (limitMB && usageMB / limitMB > options.quotaWarn) {
                    if (this.#debug) console.warn('[GoogleDrive.saveTrackingMap] Drive quota usage high', { usageMB, limitMB });
                }
            } catch (err) {
                if (this.#debug) console.warn('[GoogleDrive.saveTrackingMap] Could not check quota', err);
            }
        }
        const compress = !!options.compress;
        const shards = {};
    ...
    async set(name, value, type = "raw", options = {}) {
        if (options.quotaWarn) {
            try {
                ...
                const limitMB = parseInt(quota.limit, 10) / (1024 * 1024);
                if (limitMB && usageMB / limitMB > options.quotaWarn) {
                    if (this.#debug) console.warn('[GoogleDrive.set] Drive quota usage high', { usageMB, limitMB });
                }
            } catch (err) {
                if (this.#debug) console.warn('[GoogleDrive.set] Could not check quota', err);
            }
        }
        GoogleDrive.requireString(name, "name");
        let contentType = 'application/json';
        if (type === "raw") {
            if (typeof value !== "string" && !(value instanceof Blob)) throw new Error("Value must be a string or Blob for raw upload");
