...
export class Storage {
    ...
    static async Factory(storageRegistryPruneIntervalMs = Storage.DefaultStoragePruneIntervalMS) {
        ...
        try {
            ...
            if (storage._gitHub && typeof storage._gitHub.get === 'function') {
                gitHubOk = true;
                console.debug('[Storage.Factory] GitHubData object created:', storage._gitHub);
            } else {
                storage._gitHub = null;
                console.error('[Storage.Factory] GitHubData.factory did not return a valid object:', storage._gitHub);
            }
        } catch (e) {
            storage._gitHub = null;
        ...
        try {
            ...
            try {
                ...
                if (rawConfig && rawConfig.web) {
                    googleDriveConfig = {
                        ...
                        DISCOVERY_DOCS: rawConfig.web.discovery_docs || rawConfig.web.discovery_docs || rawConfig.web.discoveryDocs,
                ...
                console.debug('[Storage.Factory] Loaded and normalized googleDrive.json config:', googleDriveConfig);
            } catch (configErr) {
                googleDriveConfig = null;
                console.error('[Storage.Factory] Failed to load googleDrive.json config:', configErr);
            }
            const googleDrivePromise = await GoogleDrive.Factory(storage._gitHub, googleDriveConfig);
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => {
                googleDriveTimedOut = true;
                reject(new Error('[Storage.Factory] GoogleDrive.Factory timed out after 5 seconds. Proceeding without Google Drive.'));
            }, 5000));
            storage._googleDrive = await Promise.race([googleDrivePromise, timeoutPromise]);
            if (storage._googleDrive && typeof storage._googleDrive.Get === 'function' && typeof storage._googleDrive.Set === 'function') {
                googleDriveOk = true;
            } else {
            ...
            if (googleDriveTimedOut) {
                console.error('[Storage.Factory] GoogleDrive.Factory timed out. Continuing initialization without Google Drive.');
            } else {
                console.error('[Storage.Factory] Failed to create GoogleDrive object:', e);
            }
        }
        ...
        Object.defineProperty(storage, 'Get', {
            value: async function(key, options = {}) {
                if (this._sessionStorage && typeof this._sessionStorage.HasKey === 'function' && this._sessionStorage.HasKey(key)) {
                    return await this._sessionStorage.Get(key);
                }
                if (this._cache && typeof this._cache.Has === 'function' && this._cache.Has(key)) {
                    return await this._cache.Get(key);
                }
                if (this._localStorage && typeof this._localStorage.HasKey === 'function' && this._localStorage.HasKey(key)) {
        ...
        Object.defineProperty(storage, 'Set', {
            value: async function(key, value, options = {}) {
                if (this._sessionStorage && typeof this._sessionStorage.Set === 'function') {
                    await this._sessionStorage.Set(key, value);
                }
                if (this._cache && typeof this._cache.Set === 'function') {
                    await this._cache.Set(key, value);
                }
                if (this._localStorage && typeof this._localStorage.Set === 'function') {
        ...
        const finalGetCtor = storage.Get && storage.Get.constructor ? storage.Get.constructor.name : 'undefined';
        const finalSetCtor = storage.Set && storage.Set.constructor ? storage.Set.constructor.name : 'undefined';
        console.debug('[Storage.Factory][FINAL] Own property names:', finalOwnProps);
        console.debug('[Storage.Factory][FINAL] typeof Get:', finalGetType, 'constructor:', finalGetCtor);
        console.debug('[Storage.Factory][FINAL] typeof Set:', finalSetType, 'constructor:', finalSetCtor);
    ...
    static async testGoogleDrive(storage) {
        ...
        const myData = { foo: "bar", baz: 123, expires: new Date(Date.now() + 86400000).toISOString() };
        let uploadResult = null;
        try {
            // Upload with expiration and metadata
            uploadResult = await storage._googleDrive.set("mydata.json", myData, "json", {
                ...options,
        ...
        try {
            ...
            if (uploadResult && uploadResult.id) {
                const fileDownload = await storage._googleDrive.get(uploadResult.id, "json", options);
    ...
    async Get(key, options = {}) {
        ...
        let found = undefined;
        // 1. Cache
        if(this._cache && typeof this._cache.Has === 'function' && this._cache.Has(key)) found = this._cache.Get(key);
        // 2. Session Storage
        if(found === undefined && this._sessionStorage && typeof this._sessionStorage.HasKey === 'function' && this._sessionStorage.HasKey(key)) found = this._sessionStorage.Get(key);
        // 3. Local Storage
        if(found === undefined && this._localStorage && typeof this._localStorage.HasKey === 'function' && this._localStorage.HasKey(key)) found = this._localStorage.GetKey(key);
        // 4. Google Drive (if available)
        if(found === undefined && this._googleDrive && typeof this._googleDrive.HasKey === 'function' && this._googleDrive.HasKey(key)) {
            found = await this._googleDrive.Get(key, options);
        }
        // 5. GitHub
        if(found === undefined && this._gitHub && typeof this._gitHub.Has === 'function') {
