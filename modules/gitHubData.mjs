...
export class GitHubData {
     ...
     */
    constructor(repoOwner, repoName, dataPath="data", branch = "main", defaultToken = null, debug = false, options = {}) {
        ...
        if (this.#debug) {
            console.debug(`[GitHubData] Debug mode enabled.`, {
                repoOwner, repoName, dataPath, branch, defaultToken, retryCount: this.#retryCount, backoffMs: this.#backoffMs
            });
        }
    }
     ...
     * @returns {Promise<GitHubData>}
     */
    static async factory(repoOwner, repoName, dataPath="data", branch = "main", defaultToken = null, debug = false, options = {}) {
        return new GitHubData(repoOwner, repoName, dataPath, branch, defaultToken, debug, options);
    }
    ...
    async get(filename, type="raw", token = null, options = {}) {
        ...
        const normFile = GitHubData.normalizePath(filename);
        if (this.#debug) {
            console.debug(`[GitHubData.get] Fetching file:`, { filename, normFile, type, token: !!token });
        }
    ...
    async #fetchRawFile(filename, token = null, options = {}) {
        ...
        if (this.#debug) {
            console.debug(`[GitHubData.#fetchRawFile] Fetching raw:`, { filename, normFile, url, token: !!token, useToken: !!useToken });
        }
    ...
    async listDirectory(dirPath = "", token = null, options = {}) {
        ...
        if (!Array.isArray(json)) {
            if (this.#debug) console.warn(`[GitHubData.listDirectory] Non-array response, returning empty array.`, json);
            return [];
        }
        ...
        while (true) {
            try {
                ...
                if (!response.ok && response.status >= 500 && response.status < 600) {
                    ...
                    if (this.#debug) console.warn(`[GitHubData.#fetchWithRetry] 5xx error, retrying...`, { url, status: response.status, attempt });
                ...
                if (this.#debug) console.warn(`[GitHubData.#fetchWithRetry] Network error, retrying...`, { url, attempt, err });
                await new Promise(res => setTimeout(res, backoffMs * Math.pow(2, attempt)));
                attempt++;
            }
        }
