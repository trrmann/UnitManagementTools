...
export class Auth {
  constructor(config) {
    ...
    const login =
      configuration && configuration.login ? configuration.login : {};
  ...
  static async Factory(storageObject) {
    ...
    try {
      console.trace("[TRACE] Auth.Factory stack");
    } catch {}
    ...
    try {
      ...
      console.log(
        "[DEBUG] [TRACE] Dynamic import of configuration.mjs succeeded",
      );
    } catch (importErr) {
      console.error(
        "[DEBUG] [TRACE] Dynamic import of configuration.mjs FAILED:",
        importErr,
      );
      throw importErr;
    }
    ...
    console.log(
      "[DEBUG] Login modal should now be created and appended to body.",
    );
    (async function () {
      try {
      ...
      console.log(
        '[DEBUG] sessionStorage.getItem("currentUser"):',
        loggedInUser,
      );
      if (loggedInUser) {
        try {
  ...
  CreateLoginModalWithSpecs() {
    ...
    if (!this.destinationID) {
      console.warn(
        "[DEBUG] destinationID is empty or undefined! Modal will not be created.",
      );
    }
    ...
    loginSpecialAttentionSectionMessage.textContent =
      "Use any email from the dropdown with password: demo";
    ...
    loginSpecialAttentionSection.appendChild(
      loginSpecialAttentionSectionMessage,
    );
  ...
  ShowLoginForm() {
    ...
    if (loginModal) {
      ...
      console.log(
        "[DEBUG] ShowLoginForm: login modal should now be visible.",
        loginModal,
      );
    } else {
  ...
  async ShowDashboard() {
    ...
    if (this.configuration) {
      ...
      throw new Error(
        "Auth.ShowDashboard: Configuration class is not available.",
      );
    }
    ...
    if (userNameSpan && this.currentUser) {
      ...
      if (
        Array.isArray(this.currentUser.callingTitles) &&
        this.currentUser.callingTitles.length > 0
      ) {
    ...
    const _setMenuAccess = (menuClass, accessConfig) => {
      ...
      if (
        ...
        !Array.isArray(config.access[accessConfig].page)
      )
        return;
      const allowedRoleIDs = config.access[accessConfig].page;
      // Get the user's role IDs (from currentUser)
      const userRoleIDs =
        this.currentUser && Array.isArray(this.currentUser.roleIDs)
      ...
      if (
        selectedRole &&
        this.currentUser &&
        Array.isArray(this.currentUser.roleNames) &&
        Array.isArray(this.currentUser.roleIDs)
      ) {
      ...
      if (selectedRoleID !== null && allowedRoleIDs.includes(selectedRoleID)) {
        ...
        selectedRoleID === null &&
        userRoleIDs.some((id) => allowedRoleIDs.includes(id))
      ) {
        hasAccess = true;
      }
      if (!hasAccess) {
      ...
      Object.entries(parts).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          ...
          if (selectedRoleID !== null && value.includes(selectedRoleID)) {
            ...
            selectedRoleID === null &&
            userRoleIDs.some((id) => value.includes(id))
          ) {
            hasAccess = true;
          }
          partEls.forEach((el) => {
          ...
          if (value.card && Array.isArray(value.card)) {
            ...
            if (
              selectedRoleID !== null &&
              value.card.includes(selectedRoleID)
            ) {
              hasAccess = true;
            } else if (
              selectedRoleID === null &&
              userRoleIDs.some((id) => value.card.includes(id))
            ) {
              hasAccess = true;
            }
            partEls.forEach((el) => {
            ...
            if (
              ...
              if (Array.isArray(ref)) {
                ...
                if (selectedRoleID !== null && ref.includes(selectedRoleID)) {
                  ...
                  selectedRoleID === null &&
                  userRoleIDs.some((id) => ref.includes(id))
                ) {
                  hasAccess = true;
                }
                partEls.forEach((el) => {
  ...
  async LoadRoleSelector() {
    ...
    const filteredRoleNames = roleNames.filter((roleName) => {
      ...
      const matchesAnyTitle = callingTitles.some(
        (title) => (title || "").trim().toLowerCase() === normRole,
      );
    ...
    if (filteredRoleNames.length > 1) {
      if (roleSelector) {
        ...
        if (
          ...
          filteredRoleNames.includes(this.currentUser.activeRole)
        ) {
  ...
  async renderMembersTable() {
    ...
    window.changeMembersPage = function (page) {
      ...
      if (
        window.authInstance &&
        typeof window.authInstance.renderMembersTable === "function"
      ) {
