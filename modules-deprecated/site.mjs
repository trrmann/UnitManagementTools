...
export class Site {
  ...
  async openAddMember(usersInstance) {
    ...
    if (
      usersInstance &&
      usersInstance.members &&
      usersInstance.members.Roles &&
      Array.isArray(usersInstance.members.Roles.roles)
    ) {
      roles = usersInstance.members.Roles.roles.map((r) => ({
        id: r.id,
        name: r.name,
      }));
    }
      ...
      additionalRoleIDs.forEach((roleId) => {
        if (!roles.some((r) => r.id === roleId)) {
          ...
          if (
            ...
            const found = usersInstance.members.Roles.roles.find(
              (r) => r.id === roleId,
            );
