  ...
  (async () => {
    ...
    if (code && returnedState === state) {
      ...
      try {
        ...
        const secretRes = await fetch(`https://secretmanager.googleapis.com/v1/${secretResource}:access`, {
          method: 'GET',
          headers: {
        ...
        if (!secretRes.ok) {
          ...
          throw new Error(`Secret access failed (REST): ${secretRes.status} ${secretRes.statusText}\n${errText}`);
        }
        ...
        try {
          ...
          const [version] = await smClient.accessSecretVersion({ name: secretResource });
