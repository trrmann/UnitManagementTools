...
export function resetCache() {
  ...
  if (typeof window !== "undefined") {
    if (
      ...
      typeof window.Storage.Cache.clearAll === "function"
    ) {
      window.Storage.Cache.clearAll();
    } else if (
      window.CacheStore &&
      typeof window.CacheStore.clearAll === "function"
    ) {
...
export function resetSessionStorage() {
  if (typeof window !== "undefined") {
    ...
    alert(
      "Session Storage reset triggered. All session storage entries removed.",
    );
  }
}
...
export function resetCloudStorage() {
  if (typeof window !== "undefined") {
    ...
    if (
      window.CloudStorage &&
      typeof window.CloudStorage.clearAll === "function"
    ) {
...
export function attachTestingTabHandlers() {
  ...
  function getConfigInstance() {
    ...
    if (
      window.Storage &&
      window.Storage.Configuration &&
      typeof window.Storage.Configuration === "object"
    ) {
  ...
  if (resetUsersBtn)
    resetUsersBtn.onclick = async () => {
      ...
      if (window.Storage && typeof window.Storage.Set === "function") {
        ...
        await window.Storage.Set("users.json", emptyUsersObj, {
          cacheTtlMs: window.Storage._cache_default_value_expireMS,
        });
        await window.Storage.Set("users.json", emptyUsersObj, {
          sessionTtlMs: window.Storage._sessionStorage_default_value_expireMS,
        });
        await window.Storage.Set("users.json", emptyUsersObj, {
          localTtlMs: window.Storage._localStorage_default_value_expireMS,
        });
        await window.Storage.Set("users.json", emptyUsersObj, {
          googleId: "users.json",
        });
      ...
      try {
        ...
        if (usersInstance) {
          ...
          if (Array.isArray(usersInstance.UserEntries))
            usersInstance.UserEntries = [];
          if (typeof usersInstance.UsersDetails === "function") {
            usersInstance.UsersDetails = async () => [];
          }
        }
        if (window.Storage && window.Storage.Users) {
          if (Array.isArray(window.Storage.Users.users))
            window.Storage.Users.users = [];
          if (Array.isArray(window.Storage.Users.UserEntries))
            window.Storage.Users.UserEntries = [];
        ...
        if (window.Users) {
          ...
          if (Array.isArray(window.Users.UserEntries))
            window.Users.UserEntries = [];
        ...
        if (window.Storage) {
          if (
            window.Storage.Cache &&
            typeof window.Storage.Cache.Clear === "function"
          ) {
            window.Storage.Cache.Clear();
          }
          if (
            window.Storage.SessionStorage &&
            typeof window.Storage.SessionStorage.Clear === "function"
          ) {
            window.Storage.SessionStorage.Clear();
          }
          if (
            window.Storage.LocalStorage &&
            typeof window.Storage.LocalStorage.Clear === "function"
          ) {
            window.Storage.LocalStorage.Clear();
          }
          if (
            window.Storage.GoogleDrive &&
            typeof window.Storage.GoogleDrive.listDirectory === "function" &&
            typeof window.Storage.GoogleDrive.deleteFile === "function"
          ) {
            const files = await window.Storage.GoogleDrive.listDirectory(
              "name = 'users.json'",
            );
  ...
  if (viewRawUsersBtn)
    viewRawUsersBtn.onclick = async () => {
      let usersData = [];
      // Prefer UserEntries for raw entries
      if (
        window.Storage &&
        window.Storage.Users &&
        Array.isArray(window.Storage.Users.UserEntries)
      ) {
        usersData = window.Storage.Users.UserEntries;
      } else if (
        window.Storage &&
        window.Storage.Users &&
        Array.isArray(window.Storage.Users.users)
      ) {
  ...
  if (viewDetailedUsersBtn)
    viewDetailedUsersBtn.onclick = async () => {
      let detailed = null;
      const usersInstance = getUsersInstance();
      if (usersInstance) {
      ...
      if (typeof window.openModal === "function") {
        window.openModal(
          "Users (Detailed)",
          `<pre style=\"max-height:400px;overflow:auto;\">${JSON.stringify(detailed, null, 2)}</pre>`,
        );
      }
    };
  ...
  const viewDetailedMembersBtn = document.getElementById(
    "viewDetailedMembersBtn",
  );
  if (resetMembersBtn)
    resetMembersBtn.onclick = async () => {
      try {
  ...
  if (viewRawMembersBtn)
    viewRawMembersBtn.onclick = () => {
      const membersInstance = getMembersInstance();
      alert(
        "Members (Raw):\n" + JSON.stringify(membersInstance?.members, null, 2),
      );
    };
  if (viewDetailedMembersBtn)
    viewDetailedMembersBtn.onclick = () => {
      const membersInstance = getMembersInstance();
      let detailed = membersInstance;
      if (
        membersInstance &&
        typeof membersInstance.constructor.CopyToJSON === "function"
      )
  ...
  if (viewRawRolesBtn)
    viewRawRolesBtn.onclick = () => {
      const rolesInstance = getRolesInstance();
      alert("Roles (Raw):\n" + JSON.stringify(rolesInstance?.roles, null, 2));
    };
  if (viewDetailedRolesBtn)
    viewDetailedRolesBtn.onclick = () => {
      const rolesInstance = getRolesInstance();
      let detailed = rolesInstance;
      if (
        rolesInstance &&
        typeof rolesInstance.constructor.CopyToJSON === "function"
      )
  ...
  if (resetCallingsBtn)
    resetCallingsBtn.onclick = async () => {
      try {
  ...
  if (exportCallingsBtn)
    exportCallingsBtn.onclick = () => {
      const callingsInstance = getCallingsInstance();
  ...
  const resetEventScheduleTemplatesBtn = document.getElementById(
    "resetEventScheduleTemplatesBtn",
  );
  const viewRawEventScheduleTemplatesBtn = document.getElementById(
    "viewRawEventScheduleTemplatesBtn",
  );
  const viewDetailedEventScheduleTemplatesBtn = document.getElementById(
    "viewDetailedEventScheduleTemplatesBtn",
  );
  if (resetEventScheduleTemplatesBtn)
    resetEventScheduleTemplatesBtn.onclick = () => {
      alert("Mock: Reset Event Schedule Templates to GitHub values.");
    };
  if (viewRawEventScheduleTemplatesBtn)
    viewRawEventScheduleTemplatesBtn.onclick = () => {
      alert("Mock: View Raw Event Schedule Templates.");
    };
  if (viewDetailedEventScheduleTemplatesBtn)
    viewDetailedEventScheduleTemplatesBtn.onclick = () => {
  ...
  const viewDetailedWorkflowsBtn = document.getElementById(
    "viewDetailedWorkflowsBtn",
  );
  if (resetWorkflowsBtn)
    resetWorkflowsBtn.onclick = () => {
      alert("Mock: Reset Workflows to GitHub values.");
    };
  if (viewRawWorkflowsBtn)
    viewRawWorkflowsBtn.onclick = () => {
      alert("Mock: View Raw Workflows.");
    };
  if (viewDetailedWorkflowsBtn)
    viewDetailedWorkflowsBtn.onclick = () => {
  ...
  if (importCacheBtn && importCacheInput)
    importCacheBtn.onclick = () => importCacheInput.click();
  if (exportCacheBtn)
    exportCacheBtn.onclick = () => {
      let cacheData = null;
      if (
        window.Storage &&
        window.Storage.Cache &&
        typeof window.Storage.Cache.dump === "function"
      ) {
        cacheData = window.Storage.Cache.dump();
      } else if (
        window.CacheStore &&
        typeof window.CacheStore.dump === "function"
      ) {
      ...
      reader.onload = function (evt) {
        try {
          ...
          if (
            window.Storage &&
            window.Storage.Cache &&
            typeof window.Storage.Cache.load === "function"
          ) {
            window.Storage.Cache.load(data);
          } else if (
            window.CacheStore &&
            typeof window.CacheStore.load === "function"
          ) {
  ...
  // --- Session Storage ---
  const importSessionStorageInput = document.getElementById(
    "importSessionStorageInput",
  );
  const importSessionStorageBtn = document.getElementById(
    "importSessionStorageBtn",
  );
  const exportSessionStorageBtn = document.getElementById(
    "exportSessionStorageBtn",
  );
  if (importSessionStorageBtn && importSessionStorageInput)
    importSessionStorageBtn.onclick = () => importSessionStorageInput.click();
  if (exportSessionStorageBtn)
    exportSessionStorageBtn.onclick = () => {
  ...
  if (importSessionStorageInput)
    importSessionStorageInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
  ...
  // --- Local Storage ---
  const importLocalStorageInput = document.getElementById(
    "importLocalStorageInput",
  );
  const importLocalStorageBtn = document.getElementById(
    "importLocalStorageBtn",
  );
  const exportLocalStorageBtn = document.getElementById(
    "exportLocalStorageBtn",
  );
  if (importLocalStorageBtn && importLocalStorageInput)
    importLocalStorageBtn.onclick = () => importLocalStorageInput.click();
  if (exportLocalStorageBtn)
    exportLocalStorageBtn.onclick = () => {
  ...
  if (importLocalStorageInput)
    importLocalStorageInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
  ...
  // --- Cloud Storage (Mock) ---
  const importCloudStorageInput = document.getElementById(
    "importCloudStorageInput",
  );
  const importCloudStorageBtn = document.getElementById(
    "importCloudStorageBtn",
  );
  const exportCloudStorageBtn = document.getElementById(
    "exportCloudStorageBtn",
  );
  if (importCloudStorageBtn && importCloudStorageInput)
    importCloudStorageBtn.onclick = () => importCloudStorageInput.click();
  if (exportCloudStorageBtn)
    exportCloudStorageBtn.onclick = () => {
      if (
        window.CloudStorage &&
        typeof window.CloudStorage.dump === "function"
      ) {
  ...
  if (importCloudStorageInput)
    importCloudStorageInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
        try {
          ...
          if (
            window.CloudStorage &&
            typeof window.CloudStorage.load === "function"
          ) {
  ...
  const est_importRawInput = document.getElementById(
    "importRawEventScheduleTemplatesInput",
  );
  const est_exportRawBtn = document.getElementById(
    "exportRawEventScheduleTemplatesBtn",
  );
  const est_importRawBtn = document.getElementById(
    "importRawEventScheduleTemplatesBtn",
  );
  const est_exportDetailedBtn = document.getElementById(
    "exportDetailedEventScheduleTemplatesBtn",
  );
  const est_importDetailedInput = document.getElementById(
    "importDetailedEventScheduleTemplatesInput",
  );
  const est_importDetailedBtn = document.getElementById(
    "importDetailedEventScheduleTemplatesBtn",
  );

  if (est_exportRawBtn)
    est_exportRawBtn.onclick = () => {
      alert("Mock: Export Raw Event Schedule Templates triggered.");
    };
  if (est_importRawInput)
    est_importRawInput.onchange = () => {
      alert("Mock: Import Raw Event Schedule Templates triggered.");
    };
  if (est_exportDetailedBtn)
    est_exportDetailedBtn.onclick = () => {
      alert("Mock: Export Detailed Event Schedule Templates triggered.");
    };
  if (est_importDetailedInput)
    est_importDetailedInput.onchange = () => {
      alert("Mock: Import Detailed Event Schedule Templates triggered.");
    };
  if (est_importRawBtn && est_importRawInput)
    est_importRawBtn.onclick = () => est_importRawInput.click();
  if (est_importDetailedBtn && est_importDetailedInput)
    est_importDetailedBtn.onclick = () => est_importDetailedInput.click();
  ...
  const wf_exportDetailedBtn = document.getElementById(
    "exportDetailedWorkflowsBtn",
  );
  const wf_importDetailedInput = document.getElementById(
    "importDetailedWorkflowsInput",
  );
  const wf_importDetailedBtn = document.getElementById(
    "importDetailedWorkflowsBtn",
  );

  if (wf_exportRawBtn)
    wf_exportRawBtn.onclick = () => {
      alert("Mock: Export Raw Workflows triggered.");
    };
  if (wf_importRawInput)
    wf_importRawInput.onchange = () => {
      alert("Mock: Import Raw Workflows triggered.");
    };
  if (wf_exportDetailedBtn)
    wf_exportDetailedBtn.onclick = () => {
      alert("Mock: Export Detailed Workflows triggered.");
    };
  if (wf_importDetailedInput)
    wf_importDetailedInput.onchange = () => {
      alert("Mock: Import Detailed Workflows triggered.");
    };
  if (wf_importRawBtn && wf_importRawInput)
    wf_importRawBtn.onclick = () => wf_importRawInput.click();
  if (wf_importDetailedBtn && wf_importDetailedInput)
    wf_importDetailedBtn.onclick = () => wf_importDetailedInput.click();
  // --- Workflows (Mock) ---
  const importRawWorkflowsInput = document.getElementById(
    "importRawWorkflowsInput",
  );
  const exportRawWorkflowsBtn = document.getElementById(
    "exportRawWorkflowsBtn",
  );
  const importRawWorkflowsBtn = document.getElementById(
    "importRawWorkflowsBtn",
  );
  const exportDetailedWorkflowsBtn = document.getElementById(
    "exportDetailedWorkflowsBtn",
  );
  const importDetailedWorkflowsInput = document.getElementById(
    "importDetailedWorkflowsInput",
  );
  const importDetailedWorkflowsBtn = document.getElementById(
    "importDetailedWorkflowsBtn",
  );

  if (exportRawWorkflowsBtn)
    exportRawWorkflowsBtn.onclick = () => {
      alert("Mock: Export Raw Workflows triggered.");
    };
  if (importRawWorkflowsInput)
    importRawWorkflowsInput.onchange = (e) => {
      alert("Mock: Import Raw Workflows triggered.");
    };
  if (exportDetailedWorkflowsBtn)
    exportDetailedWorkflowsBtn.onclick = () => {
      alert("Mock: Export Detailed Workflows triggered.");
    };
  if (importDetailedWorkflowsInput)
    importDetailedWorkflowsInput.onchange = (e) => {
      alert("Mock: Import Detailed Workflows triggered.");
    };
  if (importRawWorkflowsBtn && importRawWorkflowsInput)
    importRawWorkflowsBtn.onclick = () => importRawWorkflowsInput.click();
  if (importDetailedWorkflowsBtn && importDetailedWorkflowsInput)
    importDetailedWorkflowsBtn.onclick = () =>
      importDetailedWorkflowsInput.click();
  // --- Users ---
  function getUsersInstance() {
    ...
    if (
      window.Storage &&
      window.Storage.Users &&
      typeof window.Storage.Users === "object"
    ) {
  ...
  const exportDetailedUsersBtn = document.getElementById(
    "exportDetailedUsersBtn",
  );
  const importDetailedUsersInput = document.getElementById(
    "importDetailedUsersInput",
  );
  const importDetailedUsersBtn = document.getElementById(
    "importDetailedUsersBtn",
  );

  // Export Raw: export users as-is
  if (exportRawUsersBtn)
    exportRawUsersBtn.onclick = () => {
      let usersData = [];
      ...
      const blob = new Blob([JSON.stringify(usersData, null, 2)], {
        type: "application/json",
      });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
  ...
  if (importRawUsersInput)
    importRawUsersInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = async function (evt) {
        try {
          ...
          const importedArr = Array.isArray(imported)
            ? imported
          ...
          if (window.Storage && typeof window.Storage.Set === "function") {
            ...
            await window.Storage.Set(
              ...
              {
                sessionTtlMs:
                  window.Storage._sessionStorage_default_value_expireMS,
              },
            );
            await window.Storage.Set(
              "users.json",
  ...
  if (exportDetailedUsersBtn)
    exportDetailedUsersBtn.onclick = async () => {
      const usersInstance = getUsersInstance();
      if (!usersInstance) {
        alert("No users found to export.");
        return;
      ...
      const blob = new Blob([JSON.stringify(detailed, null, 2)], {
        type: "application/json",
      });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
  ...
  if (importDetailedUsersInput)
    importDetailedUsersInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = async function (evt) {
        try {
          ...
          const rawData = Array.isArray(detailedData)
            ...
            : Array.isArray(detailedData.details)
              ? mockConvertDetailedToRaw(detailedData.details)
              : [];
          ...
          const sessionMS =
            storage?._sessionStorage_default_value_expireMS || 1000;
          ...
          alert(
            "Mock: Detailed users imported, converted to raw, and saved to all storages.",
          );
        } catch (err) {
  ...
  if (importRawUsersBtn && importRawUsersInput)
    importRawUsersBtn.onclick = () => importRawUsersInput.click();
  if (importDetailedUsersBtn && importDetailedUsersInput)
    importDetailedUsersBtn.onclick = () => importDetailedUsersInput.click();
  // --- Members ---
  function getMembersInstance() {
    ...
    if (
      window.Storage &&
      window.Storage.Members &&
      typeof window.Storage.Members === "object"
    ) {
  ...
  const importRawMembersInput = document.getElementById(
    "importRawMembersInput",
  );
  ...
  const exportDetailedMembersBtn = document.getElementById(
    "exportDetailedMembersBtn",
  );
  const importDetailedMembersInput = document.getElementById(
    "importDetailedMembersInput",
  );
  const importDetailedMembersBtn = document.getElementById(
    "importDetailedMembersBtn",
  );

  // Export Raw: export members as-is
  if (exportRawMembersBtn)
    exportRawMembersBtn.onclick = () => {
      const membersInstance = getMembersInstance();
  ...
  if (importRawMembersInput)
    importRawMembersInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
  ...
  if (exportDetailedMembersBtn)
    exportDetailedMembersBtn.onclick = () => {
      const membersInstance = getMembersInstance();
      if (!membersInstance) {
        alert("No members found to export.");
        return;
  ...
  if (importDetailedMembersInput)
    importDetailedMembersInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
        try {
          ...
          if (
            membersInstance &&
            typeof membersInstance.constructor.CopyFromObject === "function"
          ) {
  ...
  // Button triggers file input for import
  if (importRawMembersBtn && importRawMembersInput)
    importRawMembersBtn.onclick = () => importRawMembersInput.click();
  if (importDetailedMembersBtn && importDetailedMembersInput)
    importDetailedMembersBtn.onclick = () => importDetailedMembersInput.click();
  // --- Roles ---
  function getRolesInstance() {
    ...
    if (
      window.Storage &&
      window.Storage.Roles &&
      typeof window.Storage.Roles === "object"
    ) {
  ...
  const exportDetailedRolesBtn = document.getElementById(
    "exportDetailedRolesBtn",
  );
  const importDetailedRolesInput = document.getElementById(
    "importDetailedRolesInput",
  );
  const importDetailedRolesBtn = document.getElementById(
    "importDetailedRolesBtn",
  );

  // Export Raw: export roles as-is
  if (exportRawRolesBtn)
    exportRawRolesBtn.onclick = () => {
      const rolesInstance = getRolesInstance();
      if (!rolesInstance || !rolesInstance.roles) {
        alert("No roles found to export.");
        return;
      }
      const blob = new Blob([JSON.stringify(rolesInstance.roles, null, 2)], {
        type: "application/json",
      });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
  ...
  if (importRawRolesInput)
    importRawRolesInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
  ...
  if (exportDetailedRolesBtn)
    exportDetailedRolesBtn.onclick = () => {
      const rolesInstance = getRolesInstance();
      if (!rolesInstance) {
        alert("No roles found to export.");
        return;
  ...
  if (importDetailedRolesInput)
    importDetailedRolesInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
        try {
          ...
          if (
            rolesInstance &&
            typeof rolesInstance.constructor.CopyFromObject === "function"
          ) {
  ...
  // Button triggers file input for import
  if (importRawRolesBtn && importRawRolesInput)
    importRawRolesBtn.onclick = () => importRawRolesInput.click();
  if (importDetailedRolesBtn && importDetailedRolesInput)
    importDetailedRolesBtn.onclick = () => importDetailedRolesInput.click();
  // --- Callings ---
  function getCallingsInstance() {
    ...
    if (
      window.Storage &&
      window.Storage.Callings &&
      typeof window.Storage.Callings === "object"
    ) {
  ...
  const importRawCallingsInput = document.getElementById(
    "importRawCallingsInput",
  );
  ...
  const exportDetailedCallingsBtn = document.getElementById(
    "exportDetailedCallingsBtn",
  );
  const importDetailedCallingsInput = document.getElementById(
    "importDetailedCallingsInput",
  );
  const importDetailedCallingsBtn = document.getElementById(
    "importDetailedCallingsBtn",
  );

  // Export Raw: export callings as-is
  if (exportRawCallingsBtn)
    exportRawCallingsBtn.onclick = () => {
      const callingsInstance = getCallingsInstance();
  ...
  if (importRawCallingsInput)
    importRawCallingsInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
  ...
  if (exportDetailedCallingsBtn)
    exportDetailedCallingsBtn.onclick = () => {
      const callingsInstance = getCallingsInstance();
      if (!callingsInstance) {
        alert("No callings found to export.");
        return;
      ...
      const detailed =
        typeof callingsInstance.constructor.CopyToJSON === "function"
  ...
  if (importDetailedCallingsInput)
    importDetailedCallingsInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
        try {
          ...
          if (
            callingsInstance &&
            typeof callingsInstance.constructor.CopyFromObject === "function"
          ) {
  ...
  // Button triggers file input for import
  if (importRawCallingsBtn && importRawCallingsInput)
    importRawCallingsBtn.onclick = () => importRawCallingsInput.click();
  if (importDetailedCallingsBtn && importDetailedCallingsInput)
    importDetailedCallingsBtn.onclick = () =>
      importDetailedCallingsInput.click();
  // --- Organization ---
  function getOrgInstance() {
    ...
    if (
      window.Storage &&
      window.Storage.Organization &&
      typeof window.Storage.Organization === "object"
    ) {
  ...
  if (importRawOrgInput)
    importRawOrgInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
  ...
  if (exportDetailedOrgBtn)
    exportDetailedOrgBtn.onclick = () => {
      const orgInstance = getOrgInstance();
      if (!orgInstance) {
        alert("No organization found to export.");
        return;
      ...
      const detailed =
        typeof orgInstance.constructor.CopyToJSON === "function"
  ...
  if (importDetailedOrgInput)
    importDetailedOrgInput.onchange = (e) => {
      const file = e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = function (evt) {
        try {
          ...
          if (
            orgInstance &&
            typeof orgInstance.constructor.CopyFromObject === "function"
          ) {
  ...
  // Button triggers file input for import
  if (importRawOrgBtn && importRawOrgInput)
    importRawOrgBtn.onclick = () => importRawOrgInput.click();
  if (importDetailedOrgBtn && importDetailedOrgInput)
    importDetailedOrgBtn.onclick = () => importDetailedOrgInput.click();
  if (typeof window !== "undefined") {
    ...
    if (viewCacheBtn)
      viewCacheBtn.onclick = () => {
        let entries = [];
        if (
          window.Storage &&
          window.Storage.Cache &&
          typeof window.Storage.Cache.entries === "function"
        ) {
        ...
        reader.onload = function (evt) {
          try {
            ...
            if (
              ...
              window.Storage &&
              window.Storage.Cache &&
              typeof window.Storage.Cache.Set === "function"
            ) {
              data.forEach(([key, value]) =>
                window.Storage.Cache.Set(key, value),
              );
              alert("Cache import successful.");
            } else {
    ...
    // --- Session Storage ---
    const resetSessionStorageBtn = document.getElementById(
      "resetSessionStorageBtn",
    );
    const viewSessionStorageBtn = document.getElementById(
      "viewSessionStorageBtn",
    );
    const exportSessionStorageBtn = document.getElementById(
      "exportSessionStorageBtn",
    );
    ...
    if (resetSessionStorageBtn)
      resetSessionStorageBtn.onclick = resetSessionStorage;
    if (viewSessionStorageBtn)
      viewSessionStorageBtn.onclick = () => {
        let entries = {};
        if (window.sessionStorage) {
          for (let i = 0; i < window.sessionStorage.length; i++) {
            const key = window.sessionStorage.key(i);
    ...
    if (viewLocalStorageBtn)
      viewLocalStorageBtn.onclick = () => {
        let entries = {};
        if (window.localStorage) {
          for (let i = 0; i < window.localStorage.length; i++) {
            const key = window.localStorage.key(i);
    ...
    // --- Cloud Storage ---
    const resetCloudStorageBtn = document.getElementById(
      "resetCloudStorageBtn",
    );
    ...
    if (viewCloudStorageBtn)
      viewCloudStorageBtn.onclick = () => {
        let entries = [];
        if (
          window.CloudStorage &&
          typeof window.CloudStorage.entries === "function"
        ) {
        ...
        reader.onload = function (evt) {
          try {
            ...
            if (
              Array.isArray(data) &&
              window.CloudStorage &&
              typeof window.CloudStorage.Set === "function"
            ) {
              data.forEach(([key, value]) =>
                window.CloudStorage.Set(key, value),
              );
              alert("Cloud Storage import successful.");
            }
          } catch (err) {
    ...
    // --- Configuration ---
    const importRawConfigInput = document.getElementById(
      "importRawConfigInput",
    );
    const exportRawConfigBtn = document.getElementById("exportRawConfigBtn");
    const importDetailedConfigInput = document.getElementById(
      "importDetailedConfigInput",
    );
    const exportDetailedConfigBtn = document.getElementById(
      "exportDetailedConfigBtn",
    );
    ...
    if (exportRawConfigBtn)
      exportRawConfigBtn.onclick = () => {
        const configInstance = getConfigInstance();
    ...
    if (importRawConfigInput)
      importRawConfigInput.onchange = (e) => {
        const file = e.target.files[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onload = function (evt) {
    ...
    if (exportDetailedConfigBtn)
      exportDetailedConfigBtn.onclick = () => {
        const configInstance = getConfigInstance();
        if (!configInstance) {
          alert("No configuration found to export.");
          return;
        ...
        const detailed =
          typeof configInstance.constructor.CopyToJSON === "function"
    ...
    if (importDetailedConfigInput)
      importDetailedConfigInput.onchange = (e) => {
        const file = e.target.files[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onload = function (evt) {
          try {
            ...
            if (
              configInstance &&
              typeof configInstance.constructor.CopyFromObject === "function"
            ) {
