...
export class Members {
  ...
  get Callings() {
    return this.roles ? this.roles.Callings : undefined;
  }
  get Storage() {
    if (!this.roles || !this.roles.Callings || !this.roles.Callings.storage) {
      throw new Error(
        "Callings instance or its storage is not set on Members.",
      );
    }
  ...
  static CopyFromJSON(dataJSON) {
    ...
    members.roles = dataJSON.roles
      ? Roles.CopyFromJSON(dataJSON.roles)
      : undefined;
  ...
  static get MembersFileBasename() {
    return "members";
  }
  static get MembersFileExtension() {
    return "json";
  }
  static get MembersFilename() {
    return `${Members.MembersFileBasename}.${Members.MembersFileExtension}`;
  }
  static get MembersCacheExpireMS() {
    return 1000 * 60 * 30;
  }
  static get MembersSessionExpireMS() {
    return 1000 * 60 * 60;
  }
  static get MembersLocalExpireMS() {
    return 1000 * 60 * 60 * 2;
  }
  ...
  async Fetch() {
    if (!this.Storage) {
      throw new Error(
        "Storage is not available in Members. Ensure Roles and Callings are properly initialized.",
      );
    }
    // 1. Try to get from cache
    let membersObj = await this.Storage.Get(Members.MembersFilename, {
      ...Members.StorageConfig,
      cacheTtlMs: Members.MembersCacheExpireMS,
    });
    let foundIn = null;
    ...
    if (membersObj === undefined || membersObj === null) {
      membersObj = await this.Storage.Get(Members.MembersFilename, {
        ...Members.StorageConfig,
        cacheTtlMs: null,
        sessionTtlMs: Members.MembersSessionExpireMS,
      });
    ...
    if (
      (membersObj === undefined || membersObj === null) &&
      this.Storage &&
      ...
      membersObj = await this.Storage.Get(
        Members.MembersFilename,
        googleOptions,
      );
    ...
    if (
      ...
      try {
        membersObj = await this.Storage._gitHubDataObj.get(
          Members.MembersFilename,
          "json",
          null,
          {},
    ...
    if (membersObj !== undefined && membersObj !== null) {
      ...
      if (
        ...
        this.Storage.SessionStorage.Set(
          Members.MembersFilename,
          membersObj,
          Members.MembersSessionExpireMS,
        );
      }
  ...
  async MembersDetails() {
    ...
    const callings =
      this.roles && this.roles.Callings
        ? this.roles.Callings.CallingsDetails
        : undefined;
    ...
    this.#_membersDetails = memberEntries.map((member) => {
      ...
      const memberCallings = Array.isArray(member.callings)
        ? member.callings
        : [];
      const callingsResolved =
        memberCallings.length > 0 && this.roles && this.roles.Callings
          ? memberCallings
              .filter((callingid) =>
                this.roles.Callings.CallingIds.includes(callingid),
              )
              .map((callingid) => this.roles.Callings.CallingById(callingid))
          : [];
      const callingNames =
        callingsResolved.length > 0
          ? callingsResolved.map((callingArr) =>
              callingArr && callingArr[0] ? callingArr[0].name : null,
            )
          : [];
      const callingLevels =
        callingsResolved.length > 0
          ? callingsResolved.map((callingArr) =>
              callingArr && callingArr[0] ? callingArr[0].level : null,
            )
          : [];
      const callingsActive =
        callingsResolved.length > 0
          ? callingsResolved.map((callingArr) =>
              callingArr && callingArr[0]
                ? callingArr[0].active === true
                : false,
            )
          : [];
      const callingHaveTitles =
        callingsResolved.length > 0
          ? callingsResolved.map((callingArr) =>
              callingArr && callingArr[0] ? callingArr[0].hasTitle : null,
            )
          : [];
      const callingTitles =
        callingsResolved.length > 0
          ? callingsResolved.map((callingArr) =>
              callingArr && callingArr[0] ? callingArr[0].title : null,
            )
          : [];
      const callingTitleOrdinals =
        callingsResolved.length > 0
          ? callingsResolved.map((callingArr) =>
              callingArr && callingArr[0] ? callingArr[0].titleOrdinal : null,
            )
          : [];
      // Add callings-role-id: single array of unique role ids for all callings
      const rawCallingsRoles =
        callingsResolved.length > 0
          ? callingsResolved
              .map((callingArr) => {
      ...
      if (rawCallingsRoles.length > 0) {
        rawCallingsRoles.forEach((roleArray) => {
          roleArray.forEach((role) => {
            if (!callingsRoles.includes(role)) {
              callingsRoles.push(role);
            }
          });
        });
      }
      const callingsRoleId =
        callingsRoles.length > 0
          ? callingsRoles.map((role) => {
              return role.id;
            })
          : [];
      const callingsRoleName =
        callingsRoles.length > 0
          ? callingsRoles.map((role) => {
              return role.name;
            })
          : [];
      const callingSubRoles =
        callingsRoleId.length > 0
          ? callingsRoleId.map((callingRoleId) => {
              const role = roles.filter((role) => {
                return role.id === callingRoleId;
              })[0];
              return role ? role.subRoles : undefined;
            })
          : [];
      const callingSubRoleNames =
        callingsRoleId.length > 0
          ? callingsRoleId.map((callingRoleId) => {
              const role = roles.filter((role) => {
                return role.id === callingRoleId;
      ...
      if (member.maidenName && member.genderMale === false)
        nameParts.push(member.maidenName);
  ...
  static GetStakeLeadership(orgInstance, stakeUnitNumber) {
    if (!orgInstance || typeof orgInstance.StakeByUnitNumber !== "function")
      return null;
  ...
  static GetWardLeadership(orgInstance, wardUnitNumber) {
    if (!orgInstance || typeof orgInstance.WardByNumber !== "function")
      return null;
